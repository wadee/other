...
y.c.qe = x.Layer.extend({
	...
	Zi: function() {
		var a = this.Th = cc.LabelTTF.create("", "Arial", 12);
	...
	Hf: function(a) {
		...
		if (b.qd) this.Ii(), this.I.Vf(y.c.ne, function() {
			...
			var d = Math.abs(a);
			b.R = a;
			d < y.c.pe && (d <= y.c.Sg ? (this.Vi(), this.Ui()) : (this.Ti(), this.Kc()), this.ga.Sf(this.e), this.Xi(), this.Qi());
			d >= y.c.pe && (this.Kc(), d <= y.c.tg ? (this.If(), 0 != this.ca().R && this.Jf()) : 0 == this.ca().R ? (this.If(), this.Jf()) : this.Mi(), this.Kf())
		}
		this.na.gg(0.6, c);
		this.Li();
		this.e = g;
	...
	Pi: function() {
		...
		$.ajax({
			...
			success: function(data) {
					   ...
					   if(info.rank=="1")
					   {
						  b = _.sprintf("你把金融街盖到了%d层，和【%s】一样高，你当前累计排名第一，要保持住哦！", a, y.c.Gf());
					   }
					   else
					   {
							b = _.sprintf("你把金融街盖到了%d层，和【%s】一样高，你当前累计排名%s名，离海景房还有%s层楼，加油哦～", a, y.c.Gf(),info.rank,info.remain);
					   }
		...
		//b = _.sprintf("你把金融街盖到了%d层，和【%s】一样高，你当前累计排名XX名，离海景房还有XX层楼，加油哦～", a, y.c.Gf());
		A.Ia("end", "score", a);
		...
		function() {
			location.href="rank.php?wgateid="+getQueryString("wgateid");
		}.bind(this),"狂送500层",
		function() {
			location.href="http://xz.lifejrj.cn/erweima1";
		}.bind(this));
...
y.c.Uc = x.Y.extend({
	ctor: function(a, b, c, d) {
		...
		this.$f(e);
		//调整button的字体颜色
		a && (a = cc.LabelTTF.create(a, "黑体", 20), a.setColor(cc.color(11, 97, 131)),this.addChild(a), x.Pd(a))
	}
});
...
y.c.oe = cc.Sprite.extend({
	ctor: function(a, b, c, d, f,g,h) {
		...
		d = new y.c.Uc(d, "mtl_btn_blue", l(f), this);
		x.ea(d, this, 0.50, a-0.12);
		this.addChild(d);
		

		g = new y.c.Uc(g, "mtl_btn_blue", l(h), this);
		x.ea(g, this,0.50, a-0.24);
		this.addChild(g);
